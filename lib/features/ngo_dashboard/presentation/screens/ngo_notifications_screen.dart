...
class _NgoNotificationsScreenState extends State<NgoNotificationsScreen> {
  ...
  int _selectedTab = 1; // 0 = Subscribe, 1 = Notifications
  ...
  Future<void> _loadCategoryPreferences() async {
    ...
    try {
      ...
      setState(() {
        _subscribedCategories = (response as List)
  ...
  Future<void> _toggleCategorySubscription(String category) async {
    if (_currentUserId == null) return;
    ...
    try {
      if (_subscribedCategories.contains(category)) {
        ...
        setState(() => _subscribedCategories.remove(category));
        
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Unsubscribed from $category notifications'),
        ...
        setState(() => _subscribedCategories.add(category));
        
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Subscribed to $category notifications'),
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // Header
            _buildHeader(textMain, unreadCount),
            ...
            Expanded(
              child: _selectedTab == 0
                  ? _buildSubscribeTab(context, isDark, textMain, textSub)
                  : _buildNotificationsTab(context, isDark, textMain, textSub),
            ),
          ],
        ),
      ),
  ...
  Widget _buildHeader(Color textMain, int unreadCount) {
    return Padding(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (unreadCount > 0 && _selectedTab == 1)
                  Text(
                    '$unreadCount unread',
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 14,
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Get notified when new meals are available',
                  style: GoogleFonts.plusJakartaSans(
                    fontSize: 14,
                    color: textSub,
                  ),
          ...
          Padding(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final isSubscribed = _subscribedCategories.contains(category['name']);
  ...
  Widget _buildCategoryCard(
    ...
    return GestureDetector(
      onTap: () => _toggleCategorySubscription(name),
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: cardBg,
  ...
  Widget _buildNotificationsTab(
    ...
    if (_notifications.isEmpty && _freeMealNotifications.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
  ...
  Widget _buildFreeMealCard(
    ...
    return GestureDetector(
      onTap: () async {
        ...
        if (context.mounted) {
          // Fetch full meal data before navigating to NGO meal detail
          try {
            ...
            // Create Meal object for NGO screen
            final meal = {
              'id': mealResponse['id'],
              'title': mealResponse['title'] ?? 'Free Meal',
              'image_url': mealResponse['image_url'] ?? '',
              'discounted_price': (mealResponse['discounted_price'] as num?)?.toDouble() ?? 0.0,
              'original_price': (mealResponse['original_price'] as num?)?.toDouble() ?? 0.0,
              'quantity_available': mealResponse['quantity_available'] ?? 0,
              'expiry_date': mealResponse['expiry_date'],
              'category': mealResponse['category'] ?? '',
              'location': mealResponse['location'] ?? 'Cairo, Egypt',
              'restaurant_id': mealResponse['restaurant_id'],
