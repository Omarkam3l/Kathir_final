...
2. Tool returns data → Format into JSON response with the data
...
→ Call: build_cart(budget=500, restaurant_name="Taste of Egypt")
→ Wait for results
...
```json
{
  ...
  "data": {
    "items": [
      {"title": "Meal 1", "quantity": 2, "unit_price": 50, "subtotal": 100},
      {"title": "Meal 2", "quantity": 1, "unit_price": 75, "subtotal": 75}
    ],
    "total": 175,
...
6.  For build_cart, ALWAYS use restaurant_name="Taste of Egypt" if not specified
...
### build_cart parameters:
  ...
  restaurant_name   : use "Taste of Egypt" as default (required)
