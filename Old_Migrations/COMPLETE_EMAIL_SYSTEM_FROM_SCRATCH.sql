-- =====================================================
-- COMPLETE EMAIL SYSTEM FROM SCRATCH
-- =====================================================
-- This script completely removes all email functionality
-- and rebuilds it properly from scratch
--
-- Features:
-- - Trigger only on INSERT (not UPDATE)
...
-- =====================================================
-- STEP 1: REMOVE ALL EXISTING EMAIL FUNCTIONALITY
-- =====================================================
...
BEGIN
  ...
  -- 3. Queue email to NGO (if donation)
...
-- =====================================================
-- STEP 5: CREATE TRIGGER (INSERT ONLY!)
-- =====================================================
...
SELECT 
  'Email System Setup Complete!' as status,
  ...
  (SELECT COUNT(*) FROM pg_trigger WHERE tgname = 'trigger_queue_order_emails') as trigger_exists,
  (SELECT COUNT(*) FROM pg_proc WHERE proname = 'queue_order_email') as function_exists;

-- Show trigger definition
SELECT 
  'Trigger fires on:' as info,
  CASE 
    WHEN pg_get_triggerdef(oid) LIKE '%INSERT%UPDATE%' THEN '❌ INSERT and UPDATE (WRONG!)'
    WHEN pg_get_triggerdef(oid) LIKE '%INSERT%' THEN '✅ INSERT only (CORRECT!)'
    ELSE 'Unknown'
  END as trigger_type
FROM pg_trigger
...
COMMENT ON FUNCTION queue_order_email() IS 'Trigger function that queues emails when orders are created (INSERT only).';
...
SELECT '✅ Trigger only fires on INSERT (not UPDATE)' as performance;
SELECT '✅ Uses correct schema column names' as schema;
SELECT '✅ No duplicates will be created' as duplicates;
SELECT '✅ App should be fast again' as speed;
