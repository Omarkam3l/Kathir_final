...
// =====================================================
// Perfect email templates from Resend version
// Sending via Zoho SMTP instead of Resend API
//
// Triggered by:
...
// ✅ PERFECT TEMPLATES FROM RESEND VERSION
function generateInvoiceEmail(data: any) {
  const itemsHtml = (data.items || [])
    ...
    <tr>
      ...
      <td style="padding: 12px; border-bottom: 1px solid #e5e7eb; text-align: right;">EGP ${Number(item.unit_price).toFixed(2)}</td>
      <td style="padding: 12px; border-bottom: 1px solid #e5e7eb; text-align: right; font-weight: 600;">EGP ${Number(item.subtotal).toFixed(2)}</td>
    </tr>
  `
    )
    .join('')
