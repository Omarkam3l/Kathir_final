...
def build_cart(
    ...
    Args:
        ...
        restaurant_name   : Restaurant name (partial match allowed). REQUIRED.
    ...
    # ── Resolve restaurant (only by name, never expose IDs) ───────────────────
    rids: List[str] = []
    ...
    if not rids:
        return {"ok": False, "error": "A restaurant name is required"}
